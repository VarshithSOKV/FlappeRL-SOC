...
def find_state(bird_rect, pipe_rect):
    S = np.zeros((50,50))
    for j in range(50):
        a = pipe_rect.midbottom[0] - bird_rect.centerx
        if( a >= j*8 and a < (j+1)*8 ):
            for i in range(50):
                b = bird_rect.centery - pipe_rect.midbottom[1]
                if(b >= i*10 - 250 and b < (i+1)*10 - 250):
                    S[i][j] = 1
    return S

def next_state(pipes, bird_rect):

    pipe_rect = next_pipe(pipes)

    S = find_state(bird_rect, pipe_rect)
...
def game(Q):
    ...
    pygame.display.set_caption("Flappy Bird Game New")
    ...
    while running:
        ...
        if not game_over:
            ...
            if( k == 8 or k > 15):
                ...
                S_prime = next_state(pipes, bird_rect)
                ...
                Q[S[0]][S[1]][action] = Q[S[0]][S[1]][action] + 0.9 * (0.5 + (Q[S_prime[0]][S_prime[1]][action_prime]) - Q[S[0]][S[1]][action])
